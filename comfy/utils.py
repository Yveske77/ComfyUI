...
def resize_to_batch_size(tensor, batch_size):
    ...
    if batch_size < in_batch_size:
        ...
        indices = torch.linspace(0, batch_size - 1, batch_size, device=tensor.device, dtype=torch.float32)
        indices = (indices * scale).round().long()
        indices = indices.clamp(max=in_batch_size - 1)
    else:
        scale = in_batch_size / batch_size
        indices = torch.arange(batch_size, device=tensor.device, dtype=torch.float32)
        indices = ((indices + 0.5) * scale).floor().long()
        indices = indices.clamp(max=in_batch_size - 1)
