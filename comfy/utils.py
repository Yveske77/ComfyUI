...
def resize_to_batch_size(tensor, batch_size):
    ...
    if batch_size < in_batch_size:
        ...
        indices = torch.linspace(0, batch_size - 1, batch_size, dtype=torch.float32, device=tensor.device) * scale
        indices = torch.round(indices).long()
        indices = torch.clamp(indices, max=in_batch_size - 1)
        return tensor[indices]
    else:
        scale = in_batch_size / batch_size
        indices = (torch.arange(batch_size, dtype=torch.float32, device=tensor.device) + 0.5) * scale
