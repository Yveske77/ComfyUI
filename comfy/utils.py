...
def resize_to_batch_size(tensor, batch_size):
    ...
    if batch_size < in_batch_size:
        indices = torch.linspace(0, in_batch_size - 1, batch_size, device=tensor.device)
        indices = indices.round().long()
        indices = indices.clamp(0, in_batch_size - 1)
        return tensor[indices]
    else:
        scale = in_batch_size / batch_size
        indices = (torch.arange(batch_size, device=tensor.device) + 0.5) * scale
